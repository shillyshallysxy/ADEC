...
def train(dataset,
    ...
    dec_saver = tf.train.Saver(var_list=dec_aae_model.dec_vars+dec_aae_model.de_vars, max_to_keep=None)
    ...
    with tf.Session(config=config) as sess:
        ...
        for cur_epoch in range(100):
            ...
            for iter_, (batch_x, batch_y, batch_idxs) in enumerate(data.gen_next_batch(batch_size=batch_size,
                ...
                _, loss, pred = sess.run([dec_aae_model.train_op_dec, dec_aae_model.dec_loss, dec_aae_model.dec.pred],
                                         feed_dict=train_dec_feed)
