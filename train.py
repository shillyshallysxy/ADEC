...
def train(dataset, \
    ...
    with tf.Session(config=config) as sess:
        ...
        for cur_epoch in range(50):
            ...
            print("[DEC] epoch: {}\tloss: {}\tacc: {}".format(cur_epoch, loss, model.cluster_acc(total_y, total_pred)))
...
def eval(dataset, \
    ...
    with tf.Session(config=config) as sess:
        ...
        saver.restore(sess, dec_ckpt_path)
        total_test_y = list()
        total_test_pred = list()
        # per one epoch
        for iter_, (batch_test_x, batch_test_y, batch_test_idxs) in enumerate(data.gen_next_batch(batch_size=batch_size, \
                                                                                                  is_train_set=False,
                                                                                                  epoch=1)):
            test_pred = sess.run([model.pred], \
                                 feed_dict={model.ae.input_: batch_test_x, \
                                            model.ae.input_batch_size: batch_test_x.shape[0], \
                                            model.ae.keep_prob: 1})
            total_test_y.append(batch_test_y)
            total_test_pred.append(test_pred)
        total_test_y = np.reshape(np.array(total_test_y), [-1])
        total_test_pred = np.reshape(np.array(total_test_pred), [-1])
        print("[DEC] eval acc: {}".format(model.cluster_acc(total_test_y, total_test_pred)))


if __name__=="__main__":
    parser = configargparse.ArgParser()
    parser.add("--batch-size", dest="batch_size", help="Train Batch Size", default=64, type=int)
