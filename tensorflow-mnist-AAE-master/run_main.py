...
def parse_args():
    ...
    parser.add_argument('--prior_type', type=str, default='uniform',
                        choices=['mixGaussian', 'swiss_roll', 'normal', 'uniform'],
        ...
        for epoch in range(n_epochs):
            ...
            for i in range(total_batch):
                ...
                if args.prior_type == 'mixGaussian':
                    ...
                    samples = prior.gaussian(batch_size, dim_z, use_label_info=False)
                    z_id_ = np.random.randint(0, 10, size=[batch_size])
                elif args.prior_type == 'uniform':
                    samples = prior.uniform(batch_size, dim_z, None)
                    z_id_ = np.random.randint(0, 10, size=[batch_size])
                else:
